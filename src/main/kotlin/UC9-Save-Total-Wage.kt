...
*/
var totalWageList = mutableListOf<Int>()  //saves total wage
fun main() {
    println("Welcome to Employee Wage Computation Program")
    addCompanyToList("Jio", 20, 20, 100);
    addCompanyToList("Dmart", 20, 10, 100);
    computeWages()
}
 ...
 */
fun addCompanyToList(name: String, empWagePerHrs: Int, noOfWorkingDays: Int, maxHrsPerMonth: Int) {
 ...
 */
fun computeWageForCompanies(company: Company) {
//constants
    val isFullTime = 2
    val isPartTime = 1
    var empHrs = 0
...
fun computeWages() {
    for (company in companies) {
        computeWageForCompanies(company)
    }
}
 ...
 * @return empHrs
 */
fun getEmployeeHoursForWork(employee: EmployeeType): Int {
    var empHrs = 0
    when (employee) {
